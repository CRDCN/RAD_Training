...
<!-- Thierry comment: I made a general comment about this at the end, but I'm wondering if the
use of punctuation would be useful at the end of sentences (despite being in point form)
since they're actual phrases and no just lists of things -->
...
Can anyone discover that your data even exist? Can others even figure out where the data you used are?
...
<!-- Thierry comment: I would put the 2 questions above on different lines instead of sequentially
on the same line. That was done 2 slides over-->
...
<!-- Thierry comment: would put a comma after "that is" or just remove it altogether -->
...
**INSERT FAIR GRAPHIC**
<!-- Thierry comment: this slide is the same as that above, minus the comment. I assume this is by design-->
## Takeup (GA)
...
<!-- Thierry comment: I would add commas in b. after "isn't open"-->
## Accessibility
...
<!-- Thierry comment: this comment applies to the two above slides: it's not really clear that those are hyperlinks, at least not in the pdf on my end. Maybe changing the font colour for those would make this more obvious. This comment applies to all above and below links that are of the same font and colour-->
## Reusability
...
<!-- Thierry comment: borealis, for instance, actually attributes the PID to the dataset (cursed word) and tracks every version updates via the version tab. so the DOI never changes. Changing metadata is a minor update (1.1) and data file change is a major update (2.0)-->
...
[Screenshot of a CRDCN data page]

## Case study

[Screenshot of a CRDCN data page - highlighted]

## Case study

[Screenshot of CRDCN dataset database]
...
<!-- Thierry comment: we may want to write a guide about that at one point. If researchers are bad at one thing, it's most likely python and .R - I feel as though most just delegate to their junior members - image examples could be useful here just to depict what we're conveying-->

## Step 3: Publish

-   Post the .json file somewhere accessible on the web and share this with the Lunaris team
-   Updates made to the file will be automatically reflected in Lunaris ("Harvester" checks back on occasion)

<!-- Thierry comment: "somewhere accessible on the web" seems a bit ambiguous - could an example like GitHub be given? or something like that?-->
## Result
...
<!-- Thierry comment: I would write w/ instead of just w. Doesn't change much though. -->
...
-   Process to create/update records tedious
...
<!-- Thierry comment: i personally think the process to create stuff via borealis is pretty easy - of course I'm biased because I'm a curator. API calls also exist to automatically update the database via programs like Git, etc. --- seehttps://learn.scholarsportal.info/all-guides/borealis-admin/dataverse-api/ and https://guides.dataverse.org/en/latest/api/curation-labels.html This may be way too advanced though. But perhaps still of interest to you-->

<!-- Thierry comment: I'm pretty sure access is also limited, I looked through the documentation, but it's pretty parsimonious about what is and isn't permitted by users of non-member institutions.-->





<!-- General comment by Thierry: I'm not exactly sure if this was by design, or just the way
the slides were made, but there's no real punctuation at the end of phrases.
It is bullet point which makes sense, but I feel that it would make sense for whole sentences

It may also be a good thing to make it more obvious when there's link content. Right now it's the exact same colour as the rest of the text, which makes it difficult to know if it's a hyperlink or just text.-->
